...
export const useTask = (): UseTaskHook => {
    ...
    const updateTask = useCallback(async (id: number, completed: boolean): Promise<boolean | null> => {
        try {
            updateState({ loading: true, error: null });
            const updatedCompleted = await taskService.updateTask(id, { completed });
    ...
    const toggleTask = useCallback(async (id: number): Promise<boolean | null> => {
